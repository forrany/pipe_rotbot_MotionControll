...
void TIM3_Mode_Config(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);	   // ENCODER_GPIO时钟初始化
	...
	TIM_TimeBaseStructure.TIM_Period = 0xffff-1;  //设定计数器重装值   TIMx_ARR = 1024*4
	...
	TIM_ICInitStructure.TIM_ICFilter = 0x06;  //选择输入比较滤波器 
...
void TIM4_Mode_Config(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);	   // ENCODER_GPIO时钟初始化
	...
	TIM_DeInit(TIM4);
	TIM_TimeBaseStructInit(&TIM_TimeBaseStructure);
	TIM_TimeBaseStructure.TIM_Period = 158*4;  //设定计数器重装值   TIMx_ARR = 158*4
	TIM_TimeBaseStructure.TIM_Prescaler = 0; //TIM3时钟预分频值
	TIM_TimeBaseStructure.TIM_ClockDivision =TIM_CKD_DIV1 ;//设置时钟分割 T_dts = T_ck_int	
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up; //TIM向上计数 
...
void TIM1_Mode_Config(void)
{
	...
	//PA8 ch1  A,PA9 ch2
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1, ENABLE);	   // 注意注意注意TMDE和Timx不一样的地方 ENCODER_TIMER时钟初始化
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);	   // ENCODER_GPIO时钟初始化
	...
	TIM_DeInit(TIM1);
	TIM_TimeBaseStructInit(&TIM_TimeBaseStructure);
	TIM_TimeBaseStructure.TIM_Period = 255*4;  //设定计数器重装值   TIMx_ARR = 255*4
	TIM_TimeBaseStructure.TIM_Prescaler = 0; //TIM1时钟预分频值
	TIM_TimeBaseStructure.TIM_ClockDivision =TIM_CKD_DIV1 ;//设置时钟分割 T_dts = T_ck_int	
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up; //TIM向上计数 
	TIM_TimeBaseInit(TIM1, &TIM_TimeBaseStructure);              
                 
	TIM_EncoderInterfaceConfig(TIM1, TIM_EncoderMode_TI12, TIM_ICPolarity_Rising ,TIM_ICPolarity_Rising);//使用编码器模式3，上升下降都计数
	TIM_ICStructInit(&TIM_ICInitStructure);//将结构体中的内容缺省输入
	TIM_ICInitStructure.TIM_ICFilter = 6;  //选择输入比较滤波器 
